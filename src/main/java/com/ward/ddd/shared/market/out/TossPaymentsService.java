package com.ward.ddd.shared.market.out;
...
public class TossPaymentsService {
    ...
    public Map<String, Object> confirmCardPayment(String paymentKey, String orderId, long amount) {
        ...
        try {
            ...
            if (responseBody == null) {
                throw new DomainException(HttpStatus.BAD_REQUEST.value(), "토스 결제 승인 응답 바디가 비었습니다.");
            }
            ...
            throw new DomainException(HttpStatus.BAD_REQUEST.value(), "토스 결제 승인 호출 중 예외: " + e.getMessage());
        }
    }
    ...
    private DomainException createDomainExceptionFromNon200(int httpStatus, Map responseBody) {
        if (responseBody == null) {
            return new DomainException(HttpStatus.BAD_REQUEST.value(), "토스 결제 승인 실패(응답 바디 없음), HTTP " + httpStatus);
        }
        ...
        return new DomainException(HttpStatus.BAD_REQUEST.value(), tossCode + " - " + tossMessage);
    }
    ...
    private DomainException createDomainExceptionFromHttpError(RestClientResponseException e) {
        ...
        if (rawBody == null || rawBody.isBlank()) {
            return new DomainException(HttpStatus.BAD_REQUEST.value(), "토스 결제 승인 실패(빈 바디), HTTP " + httpStatus);
        }
        ...
        try {
            ...
            return new DomainException(HttpStatus.BAD_REQUEST.value(), "토스 결제 승인 실패, HTTP " + httpStatus + " / body=" + rawBody);
        }
    }
